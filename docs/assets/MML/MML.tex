...
% $X \cup \{ \neg \psi \}$ is inconsistent since if it was consistent, then the given set would be inconsistent since we have $\phi, \phi \implies \psi$ and modus ponens so $\psi$, contradicting $\neg \psi$. Hence by compactness there's a finite subset of the set that's inconsistent. This must include $\phi$. Therefore there are $q_1, ... , q_n, q'_1 , ... , q'_m$ so that $\phi \implies S$, where

% $$ S = \neg ( q_1 \wedge ... \wedge q_n \wedge (\neg q_1') \wedge ... \wedge ( \neg q'_m ) ) $$

% but then by definition we should have $S \in X$ and hence $q$ can't have been a valid valuation, contradiction.  

% For the final part, again use compactness. Since $X \cup \{ \neg \psi \}$ is inconsistent there's some finite subset of it that's inconsistent. Either $X$ is inconsistent, in which case $\phi$ is and therefore $\chi = \neg$ works. Alternatively, the finite subset that's inconsistent contains $\neg \psi$ and therefore we have some $x_1, ... , x_p$ so that $\phi \implies x_1 \wedge ... \wedge x_p$ and $x_1 \wedge ... \wedge x_p \implies \psi$.
% 
% Thanks to Professor Loewe for the hints.
...
\begin{remark}[The `If $A$ then $B$ or $C$' inequality.]
\label{ifAthenBC}
...
% and the the term inside the expectation is indeed $\le L_i$, and hence we get the result.
...
[More Rademacher terminology]
    The \vocab{Rademacher complexity} of a set $S \subseteq \mathbb{ R}^n$ is defined as 
...
Generalise from example sheet 2, question 11. I don't know how to get this result. %% @todo; hint is example sheet 2, question 11 for main result, and for the part involving Bayes risk somehow is due to $\phi$ being classification calibrated.
\end{proof}
\end{theorem}
...
\Return $\bar{\beta} = \frac{1}{k} \sum_{s=1}^k \beta_s$ \Comment{An average over all steps, NOT $\beta_k$}
...
This is a case where we want to consider multiple different \vocab{machine learning method}s, i.e. each $\lambda$ gives rise to a different method.
...
$M$ is called the \vocab{tuning parameter} which corresponds to the number of steps we take with AdaBoost.
...
Available at \url{arthurconmy.github.io/assets/MML/ConditionalExpectation.pdf} % one
\end{thebibliography}
\end{document}